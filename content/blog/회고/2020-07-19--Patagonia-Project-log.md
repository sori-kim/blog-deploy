...
이제는 말할 수 있을 것 같다.  
프로젝트가 끝난 후 무엇보다 중요한 건 내 코드를 다시 뜯어보며 새로 배운 것과 보완할 점에 대해 정리하는 것이지만 그때 당시에는 내 코드를 외면하고 싶었다.

욕심 많은 내가 욕심보다 훨씬 부족한 결과물을 가지고 프로젝트를 종료하게 되었다는것을 스스로 인정하고 싶지 않았던 것 같다. 하지만 내가 외면한다고 해서 내 코드가 사라지는 것도 아니고, 어쨌든 내가 뜨겁게 시간과 노력을 쏟아부었던 소중한 결과물임에는 변함 없다.
...
회고를 쓰길 잘 했다는 생각이 든다.  
그 전에는 그냥 무조건 내가 원하는 결과물을 못 얻었다는 생각에 외면하기에 급급했다. 원래 자기의 치부는 원래 들여다보고싶지 않은 법이지 않은가.  
...
무언갈 해보지 못해서 아쉬움이 든다면 그건 그냥 하면 된다.

그래서 부트캠프 수료 후 취업준비를 하면서 설정한 나만의 작은 목표는 바로,  
장바구니 기능을 Redux를 활용하여 구현해보는 것으로 정했다.  
감사하게도 프로젝트를 함께 했던 백앤드 개발자와 마음이 맞아서 마이페이지 기능을 추가하는데에 성공했고, 다른 기능들도 더 해보자고 이야기가 되었다.

가까운 미래에 전역상태관리를 드디어 할 수 있게 되었을때 기분 좋게 이 곳에 돌아와 셀프 자랑타임을 가져야겠다.  
그럼 이렇게 길었던 2차 프로젝트 회고를 마친다.
